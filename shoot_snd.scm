...
(define *ascii-bdf-file*   "data/a14.bdf")
(define *euc-bdf-file*     "data/k14.bdf")
;;(define *ascii-bdf-file* "data/12x24.bdf")
;;(define *euc-bdf-file*   "data/jiskan24.bdf")
...
(define (update-screen)
  (let ((tc (make <real-time-counter>)))
    ...
    (sys-nanosleep (abs (- 7e7 (* 1e9 (time-counter-value tc)))))
    (glut-post-redisplay)
    ))
...
(define (make-bullet n)
  (let ((state 'nop)
        ...
        (pos-y 0)
        (min-y 1)
        (fire-sound (make-sound 'laser))
        (bang-sound (make-sound 'explosion)))
    ...
    (define (fire x y)
      ...
      (set! pos-y y)
      (set! state 'flying)
      (sound-play fire-sound))

    (define (explode obj)
      (sound-stop fire-sound)
      (if (eq? obj 'enemy)
          (sound-play bang-sound))
      (set! stick *tick*)
      (set! state 'exploding))
...
(define-syntax define-sound
  (syntax-rules ()
    ((_ sym filename)
     (let ((buf (al-gen-buffer)))
       (al-buffer-data buf (alut-load-wav-file filename))
       (set! *sound-sources* (cons (cons 'sym buf) *sound-sources*))))))

(define (sound-init)
  (alut-init '())
  (define-sound laser      "data/laser.wav")
  (define-sound explosion  "data/explosion.wav")
  (define-sound crash      "data/crash.wav"))

(define (make-sound sym)
  (let ((src (al-gen-source)))
    (al-source src AL_BUFFER (cdr (assq sym *sound-sources*)))
    src))
  
(define (sound-play src)
  (al-source-play src))

(define (sound-stop src)
  (al-source-stop src))
